...
-- =============================================================================
-- Row Level Security
-- Public (anon) can READ everything. Writes are allowed for this assignment's
-- demo purposes via the anon key from the /admin dashboard — there is no
-- authentication layer. For a real deployment, gate INSERT/UPDATE/DELETE
-- behind Supabase Auth and swap these policies for `auth.role() = 'authenticated'`.
-- =============================================================================
...
create policy "public write hero"     on hero     for insert with check (true);
create policy "public update hero"    on hero     for update using (true) with check (true);

create policy "public write about"    on about    for insert with check (true);
create policy "public update about"   on about    for update using (true) with check (true);

create policy "public write skills"   on skills   for insert with check (true);
create policy "public update skills"  on skills   for update using (true) with check (true);
